...
public class ProviderChain
{
    ...
    /// <summary>
    /// Attempts to calculate transitive rate via specific intermediary
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (rate1 == null)
            {
                _logger.LogDebug("No rate found for {From}:{Intermediary}",
                    fromCurrency, intermediary);
                return null;
            }
            ...
            if (rate2 == null)
            {
                _logger.LogDebug("No rate found for {Intermediary}:{To}",
                    intermediary, toCurrency);
                return null;
            }
